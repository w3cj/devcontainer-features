...
# Check that OpenAI domains are configured (config is in firewall-config.sh)
check "openai api configured" grep -q "api.openai.com" /usr/local/bin/firewall-config.sh
