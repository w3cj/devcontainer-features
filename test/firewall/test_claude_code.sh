...
# Check that Claude Code domains are configured (config is in firewall-config.sh)
...
check "vscode marketplace configured" grep -q "marketplace.visualstudio.com" /usr/local/bin/firewall-config.sh
